...
def transformar(df, scaler, fit=False):
        """Scale time series values and convert them to a supervised
        learning format.

        Parameters
        ----------
        df : pandas.DataFrame
            DataFrame with at least the ``unidades`` column and the
            date related features.
        scaler : sklearn scaler
            Scaler used to normalise the ``unidades`` values.
        fit : bool, default False
            If ``True`` the scaler will be fitted with the supplied
            values. When using a scaler that has been fitted already
            (e.g. in production) this flag should be ``False`` so that
        ...
        # reshape because we only have one dimension
